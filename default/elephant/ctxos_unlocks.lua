--
-- Dynamic CtxOS Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the ctxos-shipped Plymouth via
-- ctxos-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs ctxos-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "ctxosunlocks"
NamePretty = "CtxOS Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/ctxos/themes"
  local ctxos_path = os.getenv("CTXOS_PATH") or ""
  local default_themes_dir = ctxos_path .. "/themes"
  local default_preview = ctxos_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "ctxos-launch-floating-terminal-with-presentation "
                .. shell_escape("ctxos-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "ctxos-launch-floating-terminal-with-presentation "
        .. shell_escape("ctxos-plymouth-reset"),
    },
  }
