...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("ctxos-launch-floating-terminal-with-presentation")
        binary = shutil.which("ctxos-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="CtxosTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("ctxos-launch-floating-terminal-with-presentation")
            and shutil.which("ctxos-transcode")
        )
