...
show_log_tail() {
  if [[ -f $CTXOS_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$CTXOS_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "CtxOS installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/ctxos/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$CTXOS_INSTALL_LOG_FILE"
      else
        tail "$CTXOS_INSTALL_LOG_FILE"
      fi
      ;;
