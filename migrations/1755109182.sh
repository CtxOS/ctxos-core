...
# Users can still opt-in to Cloudflare DNS using: ctxos-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-ctxos-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-ctxos-dns.network
  ...
  echo "To use Cloudflare DNS, run: ctxos-setup-dns Cloudflare"
fi