...
if $NEEDS_MIGRATION; then
  ...
  ctxos-pkg-drop walker-bin walker-bin-debug

  ctxos-pkg-add "${PACKAGES[@]}"

  source $CTXOS_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  ctxos-refresh-config walker/config.toml
  ctxos-refresh-config elephant/calc.toml
  ctxos-refresh-config elephant/desktopapplications.toml
fi
