...
# Move user-added backgrounds from CtxOS theme folders to user config
CTXOS_DIR="$HOME/.local/share/ctxos"
USER_BACKGROUNDS_DIR="$HOME/.config/ctxos/backgrounds"

if [[ -d $CTXOS_DIR/themes ]]; then
  cd "$CTXOS_DIR"

  # Get list of git-tracked background files (relative to ctxos dir)
...
THEMES_DIR="$HOME/.config/ctxos/themes"
CURRENT_THEME_LINK="$HOME/.config/ctxos/current/theme"
...
elif [[ -f $HOME/.config/ctxos/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/ctxos/current/theme.name")
fi

# Remove all symlinks from ~/.config/ctxos/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  ctxos-theme-set "$CURRENT_THEME_NAME"
else
